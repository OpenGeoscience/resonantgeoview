    ...
    case "number":
      if (isFinite(value)) {
        ...
        accumulated.min = Math.min(
          accumulated.min !== undefined
            ? accumulated.min
            : Number.POSITIVE_INFINITY,
          value
        );
        accumulated.max = Math.max(
          accumulated.max !== undefined
            ? accumulated.max
...
function summarize(geojson) {
  ...
  var types = {
    pointAlike: !!getFeaturesOfGeometryType(normalized, "Point", "MultiPoint")
      .length,
    lineAlike: !!getFeaturesOfGeometryType(
      normalized,
      "LineString",
      "MultiLineString"
    ).length,
    polygonAlike: !!getFeaturesOfGeometryType(
      normalized,
      "Polygon",
      "MultiPolygon"
    ).length
  ...
  var properties = accumulate(
    normalized.features.map(function(f) {
      return f.properties;
    })
  );
...
geojsonUtil.colorScale = function colorScale(
  ...
  if (!colors) {
    return function() {
      // eslint-disable-line underscore/prefer-constant
      return "#ffffff";
    };
  }
...
export { normalize, summarize, accumulate, ignoreProperties };
